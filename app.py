...
if uploaded_file is not None:
    ...
    if 'BD' in df.columns:
        ...
        if comparison_bd.shape[0] >= 2:
            ...
            pivot_table_bd['环比'] = (pivot_table_bd[latest_month] - pivot_table_bd[latest_month - 1]) / pivot_table_bd[latest_month - 1] * 100
    ...
    if 'BD' in df.columns:
        ...
        comparison_customer = monthly_data_customer[monthly_data_customer['月份'].isin([latest_month, latest_month - 1])]

        if comparison_customer.shape[0] >= 2:
            pivot_table_customer = comparison_customer.pivot(index=['客户名称', 'BD'], columns='月份', values='实付金额')
            pivot_table_customer['环比'] = (pivot_table_customer[latest_month] - pivot_table_customer[latest_month - 1]) / pivot_table_customer[latest_month - 1] * 100
